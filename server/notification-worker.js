...
function createNotificationWorker({ store, sender, notificationPayloadKey, appBaseUrl, now = () => new Date(), senderTimeoutMs = 10000, maxAttempts = 5, baseRetryMs = 60000, leaseMs = 300000 } = {}) {
  ...
  async function process(item) {
    try {
      ...
      } else if (item.notification_type === NOTIFICATION_TYPES.PASSWORD_RESET && payload.kind === 'password-reset') {
        const resetUrl = `${appBaseUrl}/website/reset-password.html#token=${encodeURIComponent(payload.resetToken)}`;
        message = { type: NOTIFICATION_TYPES.PASSWORD_RESET, to: item.recipient_email, subject: 'Atur ulang kata sandi Hamasah International', html: `<p>Assalamu'alaikum ${escapeHtml(payload.name)},</p><p>Kami menerima permintaan untuk mengatur ulang kata sandi akun Anda.</p><p><a href="${escapeHtml(resetUrl)}">Atur ulang kata sandi</a></p><p>Tautan ini memiliki masa berlaku terbatas. Jika Anda tidak memintanya, abaikan email ini.</p>` };
