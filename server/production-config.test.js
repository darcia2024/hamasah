...
// Staging/production wajib punya host absolut (tag bagikan dan sitemap, Phase R7).
const SITE = { APP_BASE_URL: 'https://hamasahinternational.com' };
...
const staging = readProductionConfig({ APP_ENV: 'staging', DATABASE_URL, IP_HASH_SECRET, ...SITE, ...STORAGE });
...
assert.throws(() => readProductionConfig({ APP_ENV: 'production', DATABASE_URL, IP_HASH_SECRET, ...SITE, ...STORAGE }), /STORAGE_BUCKET/);

// Bootstrap key dihapus setelah admin pertama dibuat, jadi production tetap jalan tanpanya.
const production = readProductionConfig({ APP_ENV: 'production', DATABASE_URL, IP_HASH_SECRET, ...SITE, ...STORAGE, STORAGE_BUCKET: 'hamasah-private-documents' });
assert.equal(production.appEnvironment, 'production');
assert.equal(production.bootstrapKey, '');

const resend = readProductionConfig({
  APP_ENV: 'production', DATABASE_URL, IP_HASH_SECRET, ...SITE, ...STORAGE, STORAGE_BUCKET: 'bucket',
...
assert.throws(() => readProductionConfig({ APP_ENV: 'production', DATABASE_URL, IP_HASH_SECRET, ...SITE, ...STORAGE, STORAGE_BUCKET: 'bucket', EMAIL_DRIVER: 'console' }), /EMAIL_DRIVER 'console'/);
...
assert.throws(() => readProductionConfig({ APP_ENV: 'production', DATABASE_URL, IP_HASH_SECRET, ...SITE, ...STORAGE, STORAGE_BUCKET: 'bucket', HAMASAH_BOOTSTRAP_KEY: 'pendek' }), /32 karakter/);
assert.throws(() => readProductionConfig({ APP_ENV: 'staging', DATABASE_URL, IP_HASH_SECRET, ...SITE, ...STORAGE, HAMASAH_BOOTSTRAP_KEY: 'pendek' }), /32 karakter/);

// IP_HASH_SECRET wajib di staging dan production, dan panjangnya diperiksa.
assert.throws(() => readProductionConfig({ APP_ENV: 'staging', DATABASE_URL, ...SITE, ...STORAGE }), /IP_HASH_SECRET/);
assert.throws(() => readProductionConfig({ APP_ENV: 'production', DATABASE_URL, ...SITE, ...STORAGE, STORAGE_BUCKET: 'bucket' }), /IP_HASH_SECRET/);
assert.throws(() => readProductionConfig({ APP_ENV: 'staging', DATABASE_URL, ...SITE, ...STORAGE, IP_HASH_SECRET: 'pendek' }), /IP_HASH_SECRET harus terdiri dari minimal 32 karakter/);
...
assert.throws(() => readProductionConfig({ APP_ENV: 'production', DATABASE_URL: 'pglite:memory', IP_HASH_SECRET, ...SITE, ...STORAGE, STORAGE_BUCKET: 'bucket' }), /pglite/);
...
assert.throws(() => readProductionConfig({ APP_ENV: 'staging', DATABASE_URL, IP_HASH_SECRET, ...SITE, STORAGE_DRIVER: 'local' }), /tidak boleh dipakai di lingkungan staging/);
assert.throws(() => readProductionConfig({ APP_ENV: 'production', DATABASE_URL, IP_HASH_SECRET, ...SITE, STORAGE_BUCKET: 'bucket', STORAGE_DRIVER: 'local' }), /tidak boleh dipakai di lingkungan production/);
...
assert.throws(() => readProductionConfig({ APP_ENV: 'staging', DATABASE_URL, IP_HASH_SECRET, ...SITE }), /SUPABASE_URL/);
assert.throws(() => readProductionConfig({ APP_ENV: 'staging', DATABASE_URL, IP_HASH_SECRET, ...SITE, SUPABASE_URL: STORAGE.SUPABASE_URL }), /SUPABASE_SERVICE_ROLE_KEY/);
...
assert.throws(() => readProductionConfig({ APP_ENV: 'production', DATABASE_URL, IP_HASH_SECRET, ...STORAGE, STORAGE_BUCKET: 'bucket' }), /APP_BASE_URL harus diisi untuk menjalankan production/);
assert.throws(() => readProductionConfig({ APP_ENV: 'staging', DATABASE_URL, IP_HASH_SECRET, ...STORAGE, APP_BASE_URL: 'ftp://x.test' }), /http atau https/);
