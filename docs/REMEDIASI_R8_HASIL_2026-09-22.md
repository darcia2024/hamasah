...
- ~~Enumerasi akun lewat waktu permintaan reset~~ **Diperbaiki 22 September 2026**: email reset dulu dikirim di dalam request hanya untuk email terdaftar (dengan pengirim 400 ms: 433 ms lawan sekitar 12 ms). Kini diantrekan ke `notification_outbox` (payload terenkripsi) dan dikirim worker; terukur 14 ms lawan 12 ms. Supaya link reset, undangan, dan notifikasi peristiwa tetap keluar di development, server dev menjalankan worker notifikasi di dalam proses (default hanya di development; `NOTIFICATION_WORKER_IN_PROCESS=true` untuk hosting satu instance). Sisa kecil: penulisan token reset ke database hanya terjadi untuk email terdaftar (beberapa ms). Bukti: `server/login-enumeration.test.js` (gagal pada route lama), `server/app-postgres.test.js`.
